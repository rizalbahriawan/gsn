...
class ProsesController extends Controller
{
    ...
    public function index(Request $request, ProsesService $prosesService)
    {
        // dd('ok');
        // dd(auth()->user()->unreadNotifications);
        $this->authorize('menuSiswa', User::class);
        $data = $prosesService->getProses();
        if($request->has('message') and !empty($request->get('message'))) {
            return view('proses.index',['data'=>$data])->with('successMessage', $request->get('message'));
        }

        // return $data->get();

        return view('proses.index',['data'=>$data]);
    }

    public function rekap(Request $request, ProsesService $prosesService)
    {
        $data = $prosesService->getRekap();

        return view('rekap.index',['data'=>$data]);
    }
    ...
    public function getProsesModal(Request $request) {
    ...
    public function store(Request $request, ProsesService $prosesService) {
        $response = array('message' => '', 'success'=>false);
        $validator = $prosesService->createValidator($request);
        if ($validator->fails()) {
            $response['message'] = $validator->messages();
            return $response;
        }
        $proses = $prosesService->store($request);
        if(!is_null($proses)) {
            // $request->session()->flash('success', 'Data proses Berhasil Diupdate!');
            return array('message' => 'Data proses berhasil diupdate', 'success'=>true, 'nama_siswa' => $proses->nama_lengkap);
        } else {
            // $request->session()->flash('error', 'Terjadi error update data proses!');
            return array('message' => 'Data proses gagal diupdate', 'success'=>false);
        }
    }
